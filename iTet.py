...
CV_HOMOGRAPHY_SRC = "518.0,646.0;650.0,38.0;1484.0,5.0;1562.0,603.0;1058.0,628.0;1017.0,150.0"
CV_HOMOGRAPHY_DST = "-203.2,127.0;0.0,127.0;0.0,-127.0;-203.2,-127.0;-203.2,0.0;-50.8,0.0"
        ...
        while running:
            ...
            if not detections:
                ...
                print(
                    ...
                    f"from world=({world_xy[0]:.1f}, {world_xy[1]:.1f}) {det.get('world_units')} "
                ...
                try:
                    ...
                    pick_z = find_deepest_feasible_z(arm_x, arm_y, PICK_Z_MM, above_z)
                    if pick_z is None:
                        print(
                            f"No feasible pick depth found between z={PICK_Z_MM:.1f} and z={above_z:.1f} "
                            f"at x={arm_x:.1f}, y={arm_y:.1f}"
                        )
                        continue
                    if abs(pick_z - PICK_Z_MM) > 1e-6:
                        print(
                            f"Requested pick depth z={PICK_Z_MM:.1f} mm was unreachable; "
