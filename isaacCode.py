...
running = True
servo_ids = [8, 5, 7]  # IDs of the three bus servos in IK order
MOVE_DURATION_S = 0.3
SERVO_ZERO_OFFSETS_DEG = {8: 90.0, 5: 90.0, 7: 90.0}
SERVO_DIRECTIONS = {8: -1.0, 5: -1.0, 7: -1.0}
SERVO_ANGLE_SCALES = {8: 1.0, 5: 1.0, 7: 1.0}
...
def capture_photo(camera):
    try:
        ...
        bgr = takePhoto.takePhoto(camera, True)
...
def _centroid_to_servo_angles(cx, cy, width, height):
    if width <= 0 or height <= 0:
        return {8: SERVO_NEUTRAL, 5: SERVO_NEUTRAL, 7: SERVO_NEUTRAL}
...
def _world_to_servo_angles(world_xy):
    ...
    wx, wy = world_xy
    xyFactor = 2
    x = (float(wx) + CV_WORLD_X_BIAS_MM)*xyFactor
    y = (float(wy) + CV_WORLD_Y_BIAS_MM)*xyFactor
    if CV_Y_ONLY_MODE:
        x = 0.0
    ...
    if angles is None:
        return None
    return {7: float(angles[0]), 8: float(angles[1]), 5: float(angles[2])}
...
def analyze_photo(image):
    try:
        if image is None:
            ...
            return None, {8: 120, 5: 120, 7: 120}

        image_with_centroid = image.copy()
        neutral = {8: SERVO_NEUTRAL, 5: SERVO_NEUTRAL, 7: SERVO_NEUTRAL}
        ...
        return image, {8: 120, 5: 120, 7: 120}
...
if __name__ == '__main__':
    ...
    if args.no_cam:
        while True:
            ...
            angles = inverseKinematics.getAngles(x, y, z)
            print(f"IK: calculated angles: {angles}")
            angles = (int(103 - int(angles[0])),) + (int(110 - int(angles[1])),) + (int(98 - int(angles[2])),)
            print(f"calculated angles: {angles}")
            angleFactor = 1/240 * 1000
            servo_commands = [[2,int(angles[1] * angleFactor)], [3, int(angles[2]*angleFactor)],[1, int(angles[0]*angleFactor)]]
    ...
    try:
        ...
        while True:
            ...
            userInput = input("new image saved, click c to skip and enter to run the cv+motors")
            if userInput == "c": continue
            time.sleep(0.5)
            ...
            if not detections:
                ...
                if not no_suction: suctionControl.turnOnSuction(ser)
                ...
                res_width = 1700 # estimation that can be improved (in mm)
                ...
                z = -550
                print(f"coordinates: {(cx, cy, z)}")
                angles = inverseKinematics.getAngles(cx, cy, z)
                if angles == None:
                    print("no feasible coordinates on above location, skipping...")
                    continue
                angles = (int(90 - int(angles[0])),) + (int(97 - int(angles[1])),) + (int(85 - int(angles[2])),)
                ...
                angles = inverseKinematics.getAngles(cx, cy, z)
                if angles == None:
                    print("no feasible coordinates on pick up location, skipping...")
                    time.sleep(2)
                    continue
                angles = (int(90 - int(angles[0])),) + (int(97 - int(angles[1])),) + (int(85 - int(angles[2])),)
                print(f"calculated angles for above position: {angles}")
                servo_commands = [[2,int(angles[1] * angleFactor)], [3, int(angles[2]*angleFactor)],[1, int(angles[0]*angleFactor)]]
                print(f"Moving servos: {servo_commands}")
                board.bus_servo_set_position(MOVE_DURATION_S * 6, servo_commands)
                time.sleep(2)
                ...
                angles = inverseKinematics.getAngles(cx, cy, z)
                if angles == None:
                    print("no feasible coordinates on drop off location, skipping...")
                    continue
                angles = (int(90 - int(angles[0])),) + (int(97 - int(angles[1])),) + (int(85 - int(angles[2])),)
                print(f"calculated angles for above position: {angles}")
                servo_commands = [[2,int(angles[1] * angleFactor)], [3, int(angles[2]*angleFactor)],[1, int(angles[0]*angleFactor)]]
                ...
                # turn off suction
                if not no_suction: suctionControl.turnOffSuction(ser)
                time.sleep(1)
